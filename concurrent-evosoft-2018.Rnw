...
comparison.64 <- data.frame(bits=baseline.v2$V1,algorithm=rep('Baseline',length(baseline.v2$V1)),evaluations=baseline.v2$value)
...
ggplot(comparison.64,aes(x=bits,y=evaluations,group=algorithm))+geom_boxplot()+theme_tufte()+labs(x="Bits",y="Evaluations",title="Concurrent individually concurrent evolutionary algorithm")
@ 
\caption{Boxplot comparing the number of evaluations needed for }
