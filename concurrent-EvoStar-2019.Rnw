...
last message and substracting the latter from the former) vs the
number of evaluations that have been performed. The result is shown in
Figure \ref{fig:evals}. Experiment with different generation gaps are
...
significative between every generation gap and the next. This
...
size and the concurrent algorithm.
...
high. It is remarkable that it is almost the same for generation gaps
higher than 16, and also slightly higher than for the minimum
generation gap that has been evaluated, 8. However, in the last two
cases the decrease in algorithmic performance more than offsets this
small speed increase in the number of evaluations per second.
...
generation gap and the algorithmic parameters. From the point of view
