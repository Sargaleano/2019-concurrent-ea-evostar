...
patterns needs to turn the algorithm in a series of stateless steps, with simple functions
...
\keywords{Concurrent algorithms,distributed computing,stateless algorithms, algorithm implementation,performance evaluation,distributed computing,heterogeneous distributed systems.}
\end{abstract}
