...
given enough time. 
Furthermore, parallel genetic algorithms (pEAs) are useful
...
Some
authors \cite{Alba2001} state that using pEAs improves the quality of
...
New parallel platforms have been identified as new trends in pEAs
...
potential use for implementing pEAs. We present two different versions
