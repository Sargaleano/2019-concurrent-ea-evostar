...
current multi-core and hyper-threaded computer architectures. However,
algorithms have to be redesigned in order to take full advantage of
the concurrent abstraction; in particular, the use of a reactive
pattern needs the algorithm to be stateless, with simple functions
that receive all the context and map it to outputs. In this paper we
...
to give a good scaling behavior and performance.
